...
(async () => {
    ...
    const fileUrl = 'https://github.com/Eaglercraft-Archive/unminified-eaglercraft-builds-1.12/releases/latest/download/Eaglercraft_1.12_Offline_en_US.html';
    const downloadedFilePath = path.resolve(__dirname, 'Eaglercraft_1.12_Offline_en_US.html');
    ...
    if (fs.existsSync(outputFilePath)) {
        ...
        if (content.includes('Modapi')) {
            console.log('[1.12] File exists and contains "Modapi".');
        } else {
            console.error('[1.12] File exists but does NOT contain "Modapi".');
